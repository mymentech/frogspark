<style>
  
  
  @media (max-width:767px) {  
    #navbar i {
      color: #fff !important;
    }
  }
  
</style>
<nav class="navbar navbar-inverse navbar-fixed-top white__bg" id="nav-wrapper">
  <div class="container">
    <div class="row">
      <div class="col-md-3" data-aos="fade-down">
        <div class="navbar-header">
          <a class="navbar-brand" href="/home"><img class="logo no__margin" src="/app/themes/frogspark/img/logo-no-vit.png" alt="Logo"></a>
          <div class="rotate-wheel__wrapper">
            <div class="rotate__wheel"></div>
          </div>
        </div>
      </div>
      <div class="col-md-9">
        <div class="hamburger__wrapper">
          <div class="hamburger__item"></div>
          <div class="hamburger__item"></div>
          <div class="hamburger__item"></div>
        </div>
        <div id="navbar">
          <ul class="nav navbar-nav">
            <li data-aos="fade-down" data-aos-delay="200"><a class="sm underline__item" href="/#how-nourish-works__section">How it works</a></li>
            <li data-aos="fade-down" data-aos-delay="300"><a class="sm underline__item" href="/#meet-nourish__section">Nourishments</a></li>
            <li data-aos="fade-down" data-aos-delay="400"><a class="sm underline__item" href="/#n-pricing">Pricing</a></li>
            <li data-aos="fade-down" data-aos-delay="200"><a class="sm underline__item" href="/#contact-cta__section_faq">FAQ</a></li>
            <li data-aos="fade-down" data-aos-delay="400"><a class="sm underline__item" href="/#contact-cta__section">Contact</a></li>
            <li data-aos="fade-down" data-aos-delay="100"><a class="sm underline__item" href="/about-us/">About Us</a></li>
            <li data-aos="fade-down" data-aos-delay="200"><a class="sm underline__item" href="/search/">
              Search <i class="fa fa-search" aria-hidden="true" style="padding-left: 2px;"></i></a></li>
            <li data-aos="fade-down" data-aos-delay="400"><a href="/#usp__section" class="sm btn white__bg green green__border nav__btn">Get Started</a></li>
<!--             <li data-aos="fade-down" data-aos-delay="400"><a href="/order/" class="sm btn green__bg white green__border nav__btn">Get Started</a></li> -->
            <li data-aos="fade-down" data-aos-delay="400"><a href="/account/index.php" class="sm btn green__bg white green__border nav__btn">My Account</a></li>            
<!--             <li data-aos="fade-down" data-aos-delay="400"><a href="/nourish-dna/" class="sm btn green__bg white green__border nav__btn">NourishDNA</a></li> -->
          </ul>
        </div>
      </div>
    </div>
  </div>
</nav>